...
*Adds .dlls from a path into the .nupkg file (which to put crudely, a zip folder with metadata)*
<Quoteblock body={"Adds .dlls from a path into the .nupkg file (which to put crudely, a zip folder with metadata)"} />

The other two item groups are msbuild variations to ensure that TargetsForTfmSpecificBuildOutput collects the correct
...
As far as you're little heart can carry you. Really that means you should look into MSBuild Targets.
A Target groups tasks to be ran together at a certain build stage. 
